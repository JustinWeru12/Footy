...
abstract class ThemeEvent {
  ...
  const factory ThemeEvent.addCustomTheme({
    required FootballThemeData themeData,
    required int themeId,
...
class _UpdateConfig extends ThemeEvent with FootballLogger {
  const _UpdateConfig({
...
class _ChangeTheme extends ThemeEvent with FootballLogger {
  const _ChangeTheme({
    this.lightThemeId,
    this.darkThemeId,
  ...
  FootballThemeData? _themeDataFromId(int? id, ThemeState state) {
    if (id == null) {
      return null;
    }
...
class _LoadCustomThemes extends ThemeEvent with FootballLogger {
  const _LoadCustomThemes();

  FootballThemeData? _decodeThemeData(String themeDataJson) {
    try {
      return FootballThemeData.fromJson(jsonDecode(themeDataJson));
    } catch (e, st) {
...
class _DeleteCustomTheme extends ThemeEvent with FootballLogger {
...
class _AddCustomTheme extends ThemeEvent with FootballLogger {
  ...
  final FootballThemeData themeData;
  final int themeId;
...
void _persistCustomThemes(List<FootballThemeData> customThemesData) {
